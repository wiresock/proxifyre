...
When the service is absent, select **Install Service**. When it is installed, **Uninstall service…** is available beside the primary service controls as well as under **Settings / Diagnostics**; uninstalling preserves configuration, its backup, and logs. Before install/start/restart, the GUI verifies that the Windows Packet Filter `NDISRD` device is actually available; a missing driver is reported immediately with the download location instead of becoming a service-start timeout. Installation invokes only the resolved engine with its fixed `install` lifecycle argument and refreshes the real Windows Service Control Manager state. Start, stop, and restart operations remain asynchronous, and a successful save is never reported as active proxying.
