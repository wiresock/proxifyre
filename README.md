...
- **The SOCKS5 server endpoint itself must be IPv4** — a literal IPv4 address, or a hostname that resolves to an **A** record. ProxiFyre connects to the proxy over IPv4 and reaches it via its IPv4-mapped address on a dual-stack socket when relaying IPv6 destinations; an **IPv6-only proxy address** (an IPv6 literal, or an AAAA-only hostname) is **not** supported. Existing IPv4 endpoints (e.g. `127.0.0.1:1080`) continue to work unchanged.
