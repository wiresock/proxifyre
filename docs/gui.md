...
- **Windows 7 UI startup failure:** use the current setup build and verify Windows 7 SP1 has current servicing-stack, SHA-2, and .NET Framework 4.7.2 updates. When both the framework default and named GDI+ font construction fail, the UI asks GDI to realize Windows' current UI font as TrueType and adopts it from a selected native device context, bypassing GDI+'s generic-family and `LOGFONT` conversion paths. A broadly damaged GDI/font subsystem can still require Windows repair.
