...
After payload validation and before constructing the first control, the managed UI verifies that .NET Framework WinForms can resolve its normal default font. If the legacy GDI+ generic-font path fails, it tries installed named Windows UI fonts and then retains the raw font created directly from Win32's default GUI font handle, avoiding GDI+'s unusable generic-family reconstruction before controls such as `DataGridView` are created. Designer typography also uses guarded family construction: healthy systems retain Segoe UI/Consolas styling, while a damaged GDI+ family lookup falls back to the recovered process font.
...
- **Windows 7 UI startup failure:** use the current setup build and verify Windows 7 SP1 has current servicing-stack, SHA-2, and .NET Framework 4.7.2 updates. The UI bypasses the legacy GDI+ generic-family path with Win32's default GUI font when both the framework default and named GDI+ font construction fail; a broadly damaged GDI/font subsystem can still require Windows repair.
